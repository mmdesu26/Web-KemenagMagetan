  ...
  useEffect(() => {
    ...
    const fetchFiles = async () => {
      try {
        ...
        if (response.success) {
          const data: UploadedFile[] = response.data
          setFiles(accept === "images" ? data.filter((file) => file.mimeType.startsWith("image/")) : data)
        }
      } catch (err) {
        console.error(err)
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[80vh]">
        ...
        <div className="space-y-4">
          ...
          {loading && <p className="text-sm text-gray-500">Memuat daftar file...</p>}
            ...
            {filteredFiles.map((file) => (
              ...
              >
                {file.mimeType.startsWith("image/") ? (
                  <img src={file.url || "/placeholder.svg"} alt={file.originalName} className="w-full h-24 object-cover rounded mb-2" />
                ) : (
...
export default function FilePicker({ onSelect, trigger, accept = "all" }: FilePickerProps) {
  ...
  const loadFiles = async () => {
    try {
      ...
      const response = await apiClient.getUploads()
      if (response.success) {
        const data: UploadedFile[] = response.data.map((file: any) => ({
          id: file.id,
